...
`detectAvailableAgents()` checks for: `.claude/` → `claude`, `.cursor/` → `cursor`, `AGENTS.md` → `codex`, `GEMINI.md` → `gemini`, `.poolside/` → `poolside`, `.vibe/` → `vibe`, `.qwen/` → `qwen`, `.kimi/` → `kimi`. This drives which agents are pre-checked in the interactive `oprim init` prompt.
