...
- **`lib/detect.ts`** — detects OpenSpec, Graphify, and AI agent environments (`.claude/`, `.cursor/`, `AGENTS.md`, `GEMINI.md`, `.poolside/`, `.vibe/`, `.qwen/`, `.kimi/`, `.dsh/`)
- **`lib/install-agent.ts`** — orchestrates which files get written to which agent directories (hooks, tombstone cleanup, PDR-surfacing Step 0 injection); the workflow *content* itself now lives in `workflow-schema.ts`/`workflow-renderer.ts`/`workflows/*` (see below). Supports `claude`, `cursor`, `codex`, `gemini`, `poolside`, `vibe`, `qwen`, `kimi`, `dsh`. Claude gets skills + command wrappers + hooks; Poolside, Mistral Vibe, Qwen Code, and DeepSeek Harness get skills + an `AGENTS.md`-style instruction file (Vibe's `vibe.skill`, Qwen's `qwen.skill`, and DeepSeek Harness's `dsh.skill` schema gates default to mirroring `poolside.skill` when a workflow's schema declares no `vibe:`/`qwen:`/`dsh:` key, so bundled/forked schemas don't need a new key to keep working); Kimi CLI is a split-path install — detected via `.kimi/` but skills written to a project-root `.skills/` directory (not `.kimi/skills/`), per Kimi's actual skill-discovery convention, plus the same `AGENTS.md`-style instruction file (its `kimi.skill` schema gate also defaults to mirroring `poolside.skill`); DeepSeek Harness writes skills directly to `.dsh/skills/` (not split-path like Kimi), per its own documented skill-discovery convention; Codex/Gemini get inline workflow text written into `AGENTS.md`/`GEMINI.md` between `<!-- oprim:start -->` / `<!-- oprim:end -->` delimiters; Cursor gets full inline command files
...
`detectAvailableAgents()` checks for: `.claude/` → `claude`, `.cursor/` → `cursor`, `AGENTS.md` → `codex`, `GEMINI.md` → `gemini`, `.poolside/` → `poolside`, `.vibe/` → `vibe`, `.qwen/` → `qwen`, `.kimi/` → `kimi`, `.dsh/` → `dsh`. This drives which agents are pre-checked in the interactive `oprim init` prompt.
