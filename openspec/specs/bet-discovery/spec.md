...
- **WHEN** `oprim/bets/BET-NNN/discovery.md` exists
...
- **WHEN** `oprim/bets/BET-NNN/discovery.md` does not exist
- **THEN** no error or failure occurs — discovery is optional

### Requirement: oprim init SHALL write a discovery.md template to oprim/templates/
The system SHALL write a `discovery.md` starter template to `oprim/templates/discovery.md` during `oprim init`, enabling teams to customize the structure.

#### Scenario: Init writes discovery template
- **WHEN** a user runs `oprim init`
- **THEN** `oprim/templates/discovery.md` is written (or refreshed) with the default discovery template structure

### Requirement: oprim doctor SHALL warn when a bet is missing discovery.md
For each directory under `oprim/bets/` that contains a `bet-decision.md`, `oprim doctor` SHALL report a non-blocking warning if `discovery.md` is absent.

#### Scenario: Doctor warns on missing discovery file
- **WHEN** `oprim/bets/BET-NNN/bet-decision.md` exists and `oprim/bets/BET-NNN/discovery.md` does not
