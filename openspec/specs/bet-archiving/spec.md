...
The system SHALL provide an `oprim:archive` skill that accepts a bare bet ID (e.g., `BET-005`), resolves it to the actual directory name in `oprim/bets/pending/` (which may include a slug, e.g., `BET-005-improve-xyz/`), moves that directory to `oprim/bets/archived/`, and removes the bet's entry from all buckets in `sequence.yaml`. If the bet directory contains an `specs/` subdirectory with delta spec files, the skill SHALL fold those deltas into the corresponding `oprim/specs/<capability>/spec.md` current-truth files (matching requirements by `### Requirement:` header text, whitespace-insensitive) before completing the move. If the bet directory contains a `tasks.md` with any unchecked `- [ ]` items, the skill SHALL warn that implementation appears incomplete and prompt the user to confirm before proceeding, rather than archiving silently. The skill has no knowledge of OpenSpec changes.

#### Scenario: Archive a bet with a slug directory
- **WHEN** a user invokes `oprim:archive` with a bare ID (e.g., BET-012) and the directory is `oprim/bets/pending/BET-012-add-title-slugs-to-bet-dirs/`
- **THEN** the skill moves `oprim/bets/pending/BET-012-add-title-slugs-to-bet-dirs/` to `oprim/bets/archived/BET-012-add-title-slugs-to-bet-dirs/`, removes the BET-012 entry from `sequence.yaml`, and reports success with the archive path

#### Scenario: Archive a bet with a legacy non-slug directory
- **WHEN** a user invokes `oprim:archive` with a bare ID (e.g., BET-005) and the directory is `oprim/bets/pending/BET-005/` (no slug)
- **THEN** the skill moves `oprim/bets/pending/BET-005/` to `oprim/bets/archived/BET-005/` and proceeds normally

#### Scenario: Archive with non-existent bet ID
- **WHEN** the user provides a bet ID that matches no directory (with or without slug) in `oprim/bets/pending/`
...
- **WHEN** a user invokes `oprim:archive BET-NNN` and `oprim/bets/pending/BET-NNN.../specs/<capability>/spec.md` exists with `## ADDED`/`## MODIFIED`/`## REMOVED Requirements` sections
...
- **WHEN** a user invokes `oprim:archive BET-NNN` and no `oprim/bets/pending/BET-NNN.../specs/` directory exists
...
#### Scenario: Archive with an incomplete tasks.md warns before proceeding
- **WHEN** a user invokes `oprim:archive BET-NNN` and `oprim/bets/pending/BET-NNN.../tasks.md` exists with at least one `- [ ]` unchecked item
- **THEN** the skill displays a warning that implementation appears incomplete, listing the count of unchecked tasks, and prompts the user to confirm before archiving

#### Scenario: Archive with a fully-checked tasks.md proceeds without warning
- **WHEN** a user invokes `oprim:archive BET-NNN` and `oprim/bets/pending/BET-NNN.../tasks.md` exists with every item checked (`- [x]`)
- **THEN** the skill archives the bet without showing an incomplete-implementation warning

#### Scenario: Archive with no tasks.md — unchanged behavior
- **WHEN** a user invokes `oprim:archive BET-NNN` and no `tasks.md` exists in the bet directory
- **THEN** the skill archives the bet exactly as before, with no task-completion check attempted
...
- **THEN** `oprim/bets/archived/BET-NNN/` contains all files that were in `oprim/bets/pending/BET-NNN/` before archiving
