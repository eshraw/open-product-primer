...
The system SHALL provide an `oprim:archive` skill that accepts a bet ID, moves the bet's directory from `oprim/bets/BET-NNN/` to `oprim/bets/archived/BET-NNN/`, and removes the bet's entry from all buckets in `sequence.yaml`. The skill has no knowledge of OpenSpec changes.

#### Scenario: Archive a completed bet
- **WHEN** a user invokes `oprim:archive` with a valid bet ID (e.g., BET-005)
- **THEN** the skill moves `oprim/bets/BET-005/` to `oprim/bets/archived/BET-005/`, removes the BET-005 entry from `sequence.yaml`, and reports success with the archive path

#### Scenario: Archive creates the archived subfolder on first use
- **WHEN** `oprim:archive` is invoked and `oprim/bets/archived/` does not yet exist
- **THEN** the skill creates the `archived/` subfolder before moving the bet directory

#### Scenario: Archive with non-existent bet ID
- **WHEN** the user provides a bet ID that has no corresponding directory in `oprim/bets/`
